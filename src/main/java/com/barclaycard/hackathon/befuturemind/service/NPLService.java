    ...
    public String hitServices(ActionResponse actionResponse) {
        ...
        switch (actionResponse.getAction()) {
            ...
            case "getAccountDetails":
                AccountDetails accountDetails = (AccountDetails) beFutureMindService.getAccount(actionResponse.getSenderId());
                ...
                {
                    return "Sorry! We couldn't find any account details associated with yourself!";

                }
            case "userDetails":
                break;
            case "getLastFiveTxn":
                List<Transaction> transactions = (List<Transaction>) beFutureMindService.getTransactions(actionResponse.getSenderId());
                System.out.println(transactions);
                if (null != transactions) {
                    String returnMessage = "Your details of last 5 transactions are as follows:\n";
                    for (int i=0;i<transactions.size();i++){
                        System.out.println(transactions.get(i));
                        //HashMap current = transactions.get(i);
                        
                       // returnMessage += i + "." + transactions.get(i).getCategory()+ " " + current.getTransactionAmount() + " " + current.getTransactionDate()+ "\n";
                    }
                    return returnMessage;
                }
                else
                {
                    return "Sorry! We couldn't find any transaction details associated with yourself!";
                }
            case "getIFSC":
                Branch branch = (Branch) beFutureMindService.getBranch(actionResponse.getSenderId());
                if (null != branch) {
                        return "Your Bank IFSC Code is - " + branch.getIFSCCode();
                }
                else
                {
                    return "Sorry! We couldn't find IFSC Code associated with your account!";

                }

            case "getBalance":
                AccountDetails accountDetails1 = (AccountDetails) beFutureMindService.getAccount(actionResponse.getSenderId());
                if (null != accountDetails1) {
                    return "Your Account Balance is - "+" Rs."+accountDetails1.getAccountBalance();
                }
                else
                {
                    return "Sorry! We couldn't find any account balance associated with yourself!";

                }
            case "followup.action":
                return "What else would you like me to do?";
            case "goodbye.action":
                System.out.println("Good bye Action");
